...
### 第一步：竞品横向对比
调用 `play_competitive_analysis(query="品类关键词")` 做全品类竞品扫描。该工具会自动：
- 搜索品类 TOP N 个 app
...
### 第二步：差评痛点分析
对前 3 个 ripe 竞品，分别调用 `play_analyze_reviews(app_id="...")`。该工具会自动：
- 批量抓取低分差评（score <= 3）
...
### 第五步：输出 Blueprint JSON
汇总所有数据，输出完整 AppOpportunityBlueprint JSON（含 requirement + product_brief）。
